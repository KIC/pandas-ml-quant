        ...
        if is_test:
            with df.model() as test_model:
                ...
                test_fit = test_model.fit(
                    SkModel(model.sk_model, FeaturesAndLabels(
                        [FEATURE_COLUMN_NAME],
                        [LABEL_COLUMN_NAME],
                        sample_weights=[SAMPLE_WEIGHTS_COLUMN_NAME] if SAMPLE_WEIGHTS_COLUMN_NAME in df else None)
                    ),
                    FittingParameter(splitter=None, epochs=1)
                )

            validation_kpis = get_kpis(test_fit.model)
