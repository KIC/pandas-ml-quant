...
class SkModel(NumpyModel):
    ...
    def __fit_model(self, x: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if getattr(self.sk_model, 'loss_', None):
            loss_curve = getattr(self.sk_model, 'loss_curve_', [])
            return np.array(loss_curve), np.array([np.nan] * len(loss_curve))
        else:
            prediction = self._predict_epoch(x)
            if isinstance(self.sk_model, LogisticRegression)\
            or type(self.sk_model).__name__.endswith("Classifier")\
            or type(self.sk_model).__name__.endswith("SVC"):
    ...
    def _predict_epoch(self, x: np.ndarray, **kwargs) -> np.ndarray:
        if callable(getattr(self.sk_model, 'predict_proba', None)):
            y_hat = self.sk_model.predict_proba(SkModel.reshape_rnn_as_ar(x))
            ...
            return self.sk_model.predict(SkModel.reshape_rnn_as_ar(x))

    def __str__(self):
        return f'{__name__}({repr(self.sk_model)}, {self.features_and_labels})'

    def __call__(self, *args, **kwargs):
        if not kwargs:
            return deepcopy(self)
        else:
            new_model = SkModel(type(self.sk_model)(**kwargs), self.features_and_labels, self.summary_provider)
