...
class TestInvestingPlugin(TestCase):
    ...
    def test_ambiguous_country(self):
        with self.assertLogs(level='WARN') as cm:
            df0 = qd.TestSymbol("AAPL", force_provider='investing')
            self.assertIn("default to united states", cm.output[-1])

    def test_dedicated_country(self):
        df0 = qd.TestSymbol("AAPL/united states", force_provider='investing')
        df1 = qd.TestSymbol("AAPL/argentina", force_provider='investing')
