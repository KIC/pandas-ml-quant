...
def download_optionable_stocks_and_sectors(sanitize_columns=True):
    ...
    def get_sector_industry(symbol):
        ...
        try:
            ...
            _log.warning(f'failed industry extraction for {symbol}: {e}')
            return None, None
        finally:
            last_page = symbol[0]

    df_si = df.apply(lambda row: get_sector_industry(str(row.name)), result_type='expand', axis=1)
